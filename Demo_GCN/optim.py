...
def train(model, model_z2g, discriminator, D_Graph, optimizers, distribution, data, device):
    ...
    values = torch.from_numpy(features[1]).to(torch.float32)
    shape = torch.Size(features[2])
    features = torch.sparse_coo_tensor(indices, values, shape).to(device)

    # transform adj_norm
    adj_norm = data['adj_norm']
    indices = torch.from_numpy(adj_norm[0].T).to(torch.long)
    values = torch.from_numpy(adj_norm[1]).to(torch.float32)
    shape = torch.Size(adj_norm[2])
    adj_norm = torch.sparse_coo_tensor(indices, values, shape)
    ...
    for _ in range(5):
        ...
        # D_graph
        z2g = model_z2g(z_real_dist, adj_norm)
    ...
    # Generator 
    latent_dim, _ = model(features, adj_norm)
