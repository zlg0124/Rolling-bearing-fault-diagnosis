...
class Encoder(nn.Module):
    def __init__(self, in_features, hidden1, hidden2, dropout=0., bias=True):
        super().__init__()

        self.layer1 = GraphConvoluationSparse(in_features, hidden1, dropout, bias)
    ...
    def forward(self, x, adj):
        ...
        embeddings = self.layer2(hidden, adj) # self.embeddings -> 2708, 128
        # self.noise = torch.randn(self.embeddings.shape, device=x.device)
        reconstructions = self.recons(embeddings)
...
if __name__ == '__main__':
    model_z2g = Generator_z2g(1433, 64, 128)
    adj = torch.randn(2708, 2708)
    x = torch.randn(2708, 128)
    print(model_z2g(x, adj).shape)
